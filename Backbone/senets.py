...
class SEResNetBottleneck(Bottleneck):
    ...
    def __init__(self, inplanes, planes, groups, reduction, stride=1,
                 downsample=None, dcn=False, dilation=1):
...
class SEResNeXtBottleneck(Bottleneck):
    ...
    def __init__(self, inplanes, planes, groups, reduction, stride=1,
                 downsample=None, base_width=4, dcn=False, dilation=1):
...
class SENet(nn.Module):
    ...
    def __init__(self, block, layers, groups, reduction, dropout_p=0.2,
                 ...
                 downsample_padding=1, dcn=(False, False, False, False),
                 dilation=[1, 1, 1, 1], stride=[1, 2, 2, 2]):
        """
        Parameters
        ----------
        ...
        if input_3x3:
            layer0_modules = [
                ...
                ('relu1', activation(inplace=True)),
                ('conv2', nn.Conv2d(64, 64, 3, stride=1, padding=1,
                                    bias=False)),
                ('bn2', nn.BatchNorm2d(64)),
                ('relu2', activation(inplace=True)),
                ('conv3', nn.Conv2d(64, inplanes, 3, stride=1, padding=1,
                                    bias=False)),
                ('bn3', nn.BatchNorm2d(inplanes)),
                ('relu3', activation(inplace=True)),
            ]
        else:
            layer0_modules = [
    ...
    def _make_layer(self, block, planes, blocks, groups, reduction, stride=1,
                    downsample_kernel_size=1, downsample_padding=0, dcn=False, dilation=1):
        downsample = None
        ...
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups, reduction, dcn=dcn, dilation=dilation))
...
def se_dilated_resnet50(num_classes=1000, pretrained=True):
    model = SENet(SEResNetBottleneck, [3, 4, 6, 3], groups=1, reduction=16,
                  dropout_p=None, inplanes=64, input_3x3=False,
                  downsample_kernel_size=1, downsample_padding=0,
                  dilation=[1, 1, 1, 2], stride=[1, 2, 2, 1])
    if pretrained:
        settings = pretrained_settings['se_resnet50']['imagenet']
        initialize_pretrained_model(model, num_classes, settings)
    return model
...
def se_dilated_resnext50_32x4d(num_classes=1000, pretrained=True):
    model = SENet(SEResNeXtBottleneck, [3, 4, 6, 3], groups=32, reduction=16,
                  dropout_p=None, inplanes=64, input_3x3=False,
                  downsample_kernel_size=1, downsample_padding=0,
                  dilation=[1, 1, 1, 2], stride=[1, 2, 2, 1])
...
def dcn101_3(num_classes=1000, pretrained=True):
    model = SENet(SEResNeXtBottleneck, [3, 4, 23, 3], groups=32, reduction=16,
                  dropout_p=None, inplanes=64, input_3x3=False,
                  downsample_kernel_size=1, downsample_padding=0,
                  dcn=(False, True, True, True))
    if pretrained:
        settings = pretrained_settings['se_resnext101_32x4d']['imagenet']
        initialize_pretrained_model(model, num_classes, settings)
    return model

def dcn101_1(num_classes=1000, pretrained=True):
    model = SENet(SEResNeXtBottleneck, [3, 4, 23, 3], groups=32, reduction=16,
                  dropout_p=None, inplanes=64, input_3x3=False,
                  downsample_kernel_size=1, downsample_padding=0,
                  dcn=(False, False, False, True))
    if pretrained:
        settings = pretrained_settings['se_resnext101_32x4d']['imagenet']
        initialize_pretrained_model(model, num_classes, settings)
    return model