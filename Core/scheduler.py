...
class CosineAnnealingLR(_LRScheduler):
    ...
    def save_best_model(self, save_dict):
        ...
        if self.save_all:
            self.save_model(fold, save_dir, state_dict, dice)
    ...
    def save_model(self, fold, save_dir, state_dict, dice):
        # Save new best model weights
        date = ':'.join(str(datetime.datetime.now()).split(':')[:2])
        if fold is not None:
            self.best_model_path = os.path.join(
                save_dir,
                '{:}_Fold{:}_Epoch{}_reset{:}_sum{:.3f}'.format(date, fold,
                                                                self.last_epoch,
                                                                self.reset,
                                                                dice))
        else:
            self.best_model_path = os.path.join(
                save_dir,
                '{:}_Epoch{}_reset{:}_dice{:.3f}'.format(date, self.last_epoch,
                                                         self.reset,
                                                         dice))
